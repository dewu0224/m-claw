...
M-Claw 面向的是**个人部署、随时使用**的场景：

- **低资源占用** — Rust 编写，单二进制文件，内存占用极低。可以跑在闲置笔记本、树莓派、或最低配的云服务器上，7×24 挂着不心疼
- **IM 即入口** — 不需要打开专门的 App 或网页，直接在你日常用的聊天工具里跟它对话。支持飞书，微信/QQ/Telegram 只需实现对应的 Channel trait 即可接入
- **移动端友好** — 部署在服务器上，手机上打开 IM 就能用。随时随地问问题、执行命令、管理文件
- **私有可控** — 数据全部存在你自己的设备上，不经过第三方平台。SQLite 本地存储，记忆文件可随时查看和编辑
